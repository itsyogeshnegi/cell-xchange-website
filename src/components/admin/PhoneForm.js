...
const fields = [
  ...
  ["imei", "IMEI / serial number", "Optional"],
];
...
export default function PhoneForm({ phone }) {
  ...
  const defaults = { category: "Phone", condition: "New", warrantyStatus: "Not specified", featured: false, latest: true, visible: true, ...phone };
  ...
  return <form onSubmit={handleSubmit(submit)}>
    ...
    <div className="grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
      <section className="rounded-[22px] border border-[#e1e4e1] bg-white p-5 sm:p-7"><h2 className="text-sm font-black">Product information</h2><div className="mt-6 grid gap-5 sm:grid-cols-2">
        <label><span className="mb-2 block text-[11px] font-bold text-[#626b64]">Product type *</span><select className="input" {...register("category", { required: true })}><option>Phone</option><option>Laptop</option><option>Smartwatch</option><option>iPad &amp; Tabs</option><option>Accessories</option></select></label>
        <label><span className="mb-2 block text-[11px] font-bold text-[#626b64]">Brand *</span><select className="input" {...register("brand", { required: true })}><option value="" disabled>Select a brand</option>{availableBrands.map((brand) => <option key={brand}>{brand}</option>)}<option value="__custom__">Other / Add custom brand</option></select></label>
        ...
        {fields.map(([name, label, placeholder, inputMode]) => <label key={name} className={name === "imei" ? "sm:col-span-2" : ""}><span className="mb-2 block text-[11px] font-bold text-[#626b64]">{label}{name === "price" ? " *" : <span className="font-normal text-[#929993]"> (optional)</span>}</span>{name === "price" ? <Controller name="price" control={control} rules={{ required: "Price is required" }} render={({ field }) => <input ref={field.ref} name={field.name} value={formatIndianNumber(field.value)} onBlur={field.onBlur} onChange={(event) => field.onChange(event.target.value.replace(/\D/g, ""))} inputMode="numeric" placeholder="1,19,900" className="input"/>}/> : name === "imei" ? <div className="flex flex-col gap-2 sm:flex-row"><input inputMode="numeric" pattern="[0-9]{15}" maxLength={15} placeholder="15-digit IMEI" className="input" {...register("imei", { pattern: { value: /^\d{15}$/, message: "IMEI must be exactly 15 digits" } })}/><button type="button" onClick={() => setScannerOpen(true)} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-[#173f2c] px-5 py-3 text-xs font-bold text-[#173f2c] transition hover:bg-[#edf4ef]"><ScanLine size={17}/>Scan barcode</button></div> : <input inputMode={inputMode} pattern={inputMode === "numeric" ? "[0-9]*" : inputMode === "decimal" ? "[0-9]+([.][0-9]+)?" : undefined} placeholder={placeholder} className="input" {...register(name)}/>} {errors[name] && <span className="mt-1 block text-[10px] text-red-600">{errors[name].message || "This field is required"}</span>}</label>)}
      ...
      <div className="grid content-start gap-6">
        ...
        <section className="rounded-[22px] border border-[#e1e4e1] bg-white p-5"><h2 className="text-sm font-black">Listing options</h2><label className="mt-5 block text-[11px] font-bold">Condition *<select className="input mt-2" {...register("condition", { required: true })}><option>New</option><option>Excellent</option><option>Good</option><option>Fair</option></select>{errors.condition && <span className="mt-1 block text-[10px] text-red-600">Condition is required</span>}</label><div className="mt-5 grid gap-4">{[["visible", "Show on website"], ["featured", "Feature on home page"], ["latest", "Mark as latest stock"]].map(([name, label]) => <label key={name} className="flex items-center justify-between text-xs font-bold"><span>{label}</span><input type="checkbox" className="h-4 w-4 accent-[#173f2c]" {...register(name)}/></label>)}</div></section>
      </div>
    </div>
    <BarcodeScannerModal open={scannerOpen} onClose={() => setScannerOpen(false)} onDetected={(value) => { setValue("imei", value, { shouldDirty: true, shouldValidate: true }); toast.success("IMEI scanned and filled"); }}/>
  </form>;
}
