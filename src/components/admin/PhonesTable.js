...
import { Edit3, FileSpreadsheet, Plus, Search, Trash2 } from "lucide-react";
import { useCallback, useDeferredValue, useEffect, useRef, useState, useTransition } from "react";
import ExportModal from "@/components/admin/ExportModal";
import { formatPrice } from "@/utils/format";

const statusStyles = {
  Available: "bg-[#edf7ef] text-[#246441]",
  Sold: "bg-[#fff0eb] text-[#a34b32]",
  Block: "bg-[#f1f1f1] text-[#555b57]",
};

export default function PhonesTable({ initialPage }) {
  ...
  useEffect(() => {
    const signature = `${deferredQuery}:${status}:${page}`;
  ...
  const changeStatus = useCallback((event) => startTransition(() => { setStatus(event.target.value); setPage(1); }), []);
  ...
  return <>
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <p className="eyebrow text-[#718078]">All inventory</p>
        <h1 className="display mt-2 text-4xl font-black">Products</h1>
        <p className="mt-2 text-sm text-[#747c76]">Manage phones, laptops and smartwatches from one place.</p>
      </div>
      <div className="flex flex-wrap gap-3">
        <button onClick={() => setIsExportOpen(true)} className="inline-flex items-center justify-center gap-2 rounded-full border border-[#cbd1cc] bg-white px-5 py-3 text-xs font-bold text-[#151915] hover:bg-[#f4f5f3]">
          <FileSpreadsheet size={16} className="text-[#173f2c]" />
          Export Excel
        </button>
        <Link href="/dashboard/phones/create" className="inline-flex items-center justify-center gap-2 rounded-full bg-[#173f2c] px-5 py-3 text-xs font-bold text-white">
