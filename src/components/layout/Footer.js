  ...
  return <footer className="bg-[#0b0c0e] text-white">
    <div className="container-shell py-16 sm:py-20">
      <div className="grid gap-12 border-b border-white/10 pb-14 lg:grid-cols-[1.2fr_.75fr_.9fr]">
        <div><Logo light logo={profile.brandLogo} name={profile.name}/><p className="mt-6 max-w-md text-sm leading-7 text-white/50">{profile.footerDescription}</p><a href={profile.whatsappUrl} target="_blank" rel="noreferrer" className="mt-7 inline-flex items-center gap-2 border-b border-white/30 pb-1 text-xs font-bold uppercase tracking-[.12em]">Start a conversation <ArrowUpRight size={14}/></a></div>
