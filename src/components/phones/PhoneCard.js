...
export default function PhoneCard({ phone, compact = false }) {
  ...
  return <article className="group border border-[#e0e1df] bg-white transition duration-300 hover:border-[#adafb4] hover:shadow-[0_18px_45px_rgba(15,17,20,.08)]">
    <Link href={href} className={`relative block overflow-hidden bg-[#f1f1ef] ${compact ? "h-[240px] sm:h-[280px] lg:h-[320px]" : "aspect-[1.02]"}`}><Image src={phone.images?.[0]?.url} alt={`${phone.brand} ${phone.model}`} fill quality={90} sizes={compact ? "(max-width: 640px) 86vw, (max-width: 1024px) 62vw, 52vw" : "(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw"} className={`${compact ? "object-contain p-5 sm:p-7" : "object-cover"} transition duration-700 group-hover:scale-[1.025]`}/><div className="absolute left-4 top-4 flex flex-wrap gap-2"><span className={`border px-2.5 py-1.5 text-[9px] font-bold uppercase tracking-[.13em] backdrop-blur ${available ? "border-white/70 bg-white/85 text-black" : "border-black bg-black text-white"}`}>{status}</span></div></Link>
