...
export default function PhoneGallery({ phone }) {
  ...
  return <div className="grid gap-3 sm:grid-cols-[76px_1fr]">
    ...
    <button onClick={()=>setZoom(!zoom)} className={`group relative order-1 overflow-hidden rounded-[28px] bg-[#f3f4f2] sm:order-2 ${zoom?"cursor-zoom-out":"cursor-zoom-in"}`} style={{aspectRatio:"1/1.08"}}><Image fill priority quality={90} src={images[active]?.url} alt={`${phone.brand} ${phone.model}`} sizes="(max-width: 768px) 100vw, 50vw" className={`object-cover transition duration-500 ${zoom?"scale-150":"group-hover:scale-[1.03]"}`}/><span className="absolute right-4 top-4 grid h-10 w-10 place-items-center rounded-full bg-white/90 shadow-sm"><Maximize2 size={16}/></span></button>
  </div>;
}
