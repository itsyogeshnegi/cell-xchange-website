...
export default function Catalog({ initialPage, facets }) {
  ...
  return <>
    ...
    <div className="mb-5 flex items-center justify-between" aria-live="polite"><p className={`text-sm text-[#69716c] transition-opacity ${pending ? "opacity-50" : "opacity-100"}`}><strong className="text-[#151915]">{total}</strong> products available</p>{(query || brand !== "All" || storage !== "All") && <button onClick={clear} className="flex items-center gap-1 text-xs font-bold"><X size={14}/> Clear filters</button>}</div>
