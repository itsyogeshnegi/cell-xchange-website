...
export default function Catalog({ initialPage, facets }) {
  ...
  return <>
    ...
    {pages > 1 && <div className="mt-10 flex justify-center gap-2">{Array.from({ length: pages }, (_, index) => index + 1).map((item) => <button key={item} onClick={() => startTransition(() => setPage(item))} className={`h-10 w-10 rounded-full text-sm font-bold ${page === item ? "bg-[#173f2c] text-white" : "border border-[#e1e4e1]"}`}>{item}</button>)}</div>}
  </>;
}
