...
export const publicPhoneFields = "brand model slug description price discount color storage ram battery processor display camera condition stock featured latest images createdAt updatedAt";
...
export function parsePhoneForm(form) {
  ...
  return {
    ...
    stock: number("stock"), featured: form.get("featured") === "true", latest: form.get("latest") === "true",
  };
}

export function validateImageFiles(files, required = true) {
  if (required && !files.length) throw Object.assign(new Error("At least one image is required"), { status: 422 });
