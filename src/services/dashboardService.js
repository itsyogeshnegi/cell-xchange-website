...
const readStats = unstable_cache(async () => {
  ...
  const [stats] = await Phone.aggregate([
    { $facet: {
      totals: [{ $group: { _id: null, total: { $sum: 1 }, totalCost: { $sum: { $multiply: ["$price", "$stock"] } }, units: { $sum: "$stock" }, available: { $sum: { $cond: [{ $gt: ["$stock", 0] }, 1, 0] } }, outOfStock: { $sum: { $cond: [{ $eq: ["$stock", 0] }, 1, 0] } } } }],
      recent: [{ $sort: { createdAt: -1 } }, { $limit: 5 }, { $project: { brand: 1, model: 1, storage: 1, price: 1, stock: 1, images: { $slice: ["$images", 1] }, createdAt: 1 } }],
    } },
    { $project: { recent: 1, total: { $ifNull: [{ $first: "$totals.total" }, 0] }, totalCost: { $ifNull: [{ $first: "$totals.totalCost" }, 0] }, units: { $ifNull: [{ $first: "$totals.units" }, 0] }, available: { $ifNull: [{ $first: "$totals.available" }, 0] }, outOfStock: { $ifNull: [{ $first: "$totals.outOfStock" }, 0] } } },
