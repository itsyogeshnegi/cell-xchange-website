...
import { applyDeviceFilter, matchesDeviceFilter } from "@/lib/device-filters";
...
export async function getPhonePage({ page = 1, limit = 12, q = "", brand = "", storage = "", sort = "newest", device = "", admin = false } = {}) {
  ...
  if (!process.env.MONGODB_URI) {
    let items = demoPhones.filter((phone) => (!q || `${phone.brand} ${phone.model}`.toLowerCase().includes(q.toLowerCase())) && (!brand || phone.brand === brand) && (!storage || phone.storage === storage) && matchesDeviceFilter(phone, device));
